import { switchVerification } from "@/reducers/users";
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/users")
      ...
      .then((data) => {
        ...
        setUsers(
          data.users.filter(
            (user) => user.company.length > 0 && user.verification === false
          )
        );
      })
          ...
          users.map((user) =>
            user._id === userId
              ? {
                  ...
                  company: verificationStatus ? user.company : [],
                }
              : user
          )
        );
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      {!isLoggedIn ? (
        ...
        <div className="bg-white p-8 rounded-lg shadow-xl w-[500px]">
          ...
          <p className="py-5 underline font-semibold text-center text-gray-700">
            Personnes en attente de vérification
          </p>
          <ul className="space-y-4">
            {users.map((user) => (
              <li
                key={user._id}
                className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg bg-gray-50 hover:bg-gray-100 transition"
              >
                <div className="flex-1">
                  <p className="block text-lg text-gray-800">
                    <span className="font-bold mb-2">Nom:</span>{" "}
                  ...
                  {user.company.length > 0 && (
                    <p className="text-gray-600 mb-2">
                      <span className="font-bold">Nom de l'entreprise:</span>{" "}
